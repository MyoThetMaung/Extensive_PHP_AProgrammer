            ...
            <div class="card">
              ...
              <?php
                  ...
                  if(empty($_POST['search'])){
                    ...
                    $statement = $pdo->prepare("SELECT * FROM products LIMIT $offset,$num_of_record");
                    $statement->execute();
                    $result = $statement->fetchAll();
                  }else{
                    $search_key = $_POST['search'];
                    $statement = $pdo->prepare("SELECT * FROM products WHERE name LIKE '%$search_key%' ORDER BY id DESC");
                    $statement->execute();
                    $raw_result = $statement->fetchAll();
                    $total_page = ceil(count($raw_result)/$num_of_record);
                  
                    $statement = $pdo->prepare("SELECT * FROM products WHERE name LIKE '%$search_key%' LIMIT $offset,$num_of_record");
                    $statement->execute();
                    $result = $statement->fetchAll();
                  }
              ?>
              ...
              <div class="card-body">
                <table class="table table-bordered">
                  <a href="product_add.php" type="button" class="btn btn-success">New Product Add</a> <br> <br>
                  <thead>
                    <tr>
                  ...
                  if ($result) {
                    foreach ($result as $value) { 
                      $statement = $pdo->prepare("SELECT * FROM categories WHERE id=".$value['category_id']);
                      $statement->execute();
                      ...
                      <tr>
                        ...
                        <td>
                            <div class="d-flex">
                              <a href="product_edit.php?id=<?php echo $value['id']; ?>" class="btn btn-warning m-1" type="button">Edit</a>                
                                <a href="product_delete.php?id=<?php echo $value['id']; ?>" class="btn btn-danger m-1" type="button" onclick="return confirm('will you delete?');">Delete</a>                   
                            </div>
                        </td>
                        </tr>
                  <?php
