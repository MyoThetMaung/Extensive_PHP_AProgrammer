      ...
      if($quantity_error == '' && $price_error ==''){
        if($_FILES['image']['name'] != null){
            ...
            if($image_type != 'jpeg' && $image_type != 'png' && $image_type != 'jpg'){
                ...
                $statement = $pdo->prepare("UPDATE products SET name='$name', description='$description', 
                                                  category_id='$category_id',price='$price',quantity='$quantity',image='$image'
                                                  WHERE id=".$_POST['id']);
            ...
            $author_id = $_SESSION['user_id'];
            $statement = $pdo->prepare("UPDATE products SET name='$name', description='$description', 
                                                  category_id='$category_id',price='$price',quantity='$quantity' 
                                                  WHERE id=".$_POST['id']);
            $result = $statement->execute();
            if ($result) {
                echo "<script>alert('Product updated successfully!'); window.location.href='index.php';</script>";
              ...
              <form action="product_edit.php" method="POST" enctype="multipart/form-data">
                ...
                <input type="hidden" name="id" value="<?php echo $result['id']; ?>">
                  ...
                  <div class="form-group">
                      ...
                      <textarea type="text" name="description" rows="8" cols="50" class="form-control"><?php echo $result['description']; ?></textarea>
                  </div>
                    ...
                    ?>
                      ...
                      <select name="category" class="form-control">
                        ...
                        <?php foreach ($cat_result as $value) { ?>
                          ...
                          <option <?php echo $value['id']==$result['id'] ? 'selected' : ""; ?> value="<?php echo $value['id']; ?>"><?php echo $value['name']; ?></option>
                  ...
                  <div class="form-group">
                      ...
                      <img src="images/<?php echo $result['image']; ?>" alt="" width="200" height="200">
